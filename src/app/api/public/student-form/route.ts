...
export async function POST(req: NextRequest) {
  try {
    const { parentName, parentEmail, name, grade, level, dob, parentContact1, parentContact2, selfPickupSession } = await req.json()
    ...
    const { pending, redirectUrl } = await registrationService.createPendingRegistration({
      ...
      studentName: name, studentGrade: grade, studentLevel: level, dob, selfPickupSession,
